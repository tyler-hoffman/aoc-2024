...
class Day20PartASolver:
    ...
    def cheats(self) -> Iterator[int]:
        STEPS_DURING_CHEAT = 2
        for p, dist in self.dists_to_end.items():
            for n in self.second_neighbors(p):
                if n in self.non_walls:
                    yield dist - self.dists_to_end[n] - STEPS_DURING_CHEAT
...
def solve(input: str, min_seconds_to_save: int = 100) -> int:
    ...
    solver = Day20PartASolver(data, min_seconds_to_save=min_seconds_to_save)
